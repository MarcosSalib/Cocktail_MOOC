...
class BestFirstSearch(SearchBaseClass, ABC):
    ...
    def execute_search(self) -> Tuple[Union[None, List[List[State]]], Union[None, List[MotionPrimitive]], Any]:
        ...
        while not self.frontier.empty():
            ...
            # filtering nodes when depth_tree is too high
            if node_current.depth_tree > 40:
                removed_nodes = []
                for node in self.frontier.list_elements:
                    if node[-1].depth_tree < (node_current.depth_tree * 0.1 ):
                        removed_nodes.append(self.frontier.list_elements.index(node))

                if removed_nodes:
                    for i in reversed(removed_nodes):
                        self.frontier.list_elements.pop(i)
