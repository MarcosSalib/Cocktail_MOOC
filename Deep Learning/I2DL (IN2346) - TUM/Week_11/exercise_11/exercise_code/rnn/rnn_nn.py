...
class LSTM(nn.Module):
    ...
    def forward(self, x, h=None, c=None):
        ...
        for xt in x.unbind(0):
            # update the hidden state
            f_t = self.act_sig(self.W_hh(h) + self.W_xh(xt))
            i_t = self.act_sig(self.W_hh(h) + self.W_xh(xt))
            o_t = self.act_sig(self.W_hh(h) + self.W_xh(xt))

            c = (f_t * c) + (i_t * self.act_tanh(self.W_hh(h) + self.W_xh(xt)))
            h = o_t * self.act_tanh(c)
...
class Embedding(nn.Module):
    def __init__(self, num_embeddings, embedding_dim, padding_idx):
        ...
        self.weight = torch.normal(0, 1, (self.num_embeddings, self.embedding_dim))
